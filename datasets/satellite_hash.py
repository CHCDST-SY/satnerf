...
class SatelliteDataset_hash(Dataset):
    def __init__(
        self, root_dir, img_dir, split="train", img_downscale=1.0, cache_dir=None
    ):
        """
        ...
        self.bounding_box = torch.tensor(d["bounding_box"])
    ...
    def load_val_split(self):
        ...
        # Preload and cache validation samples to avoid heavy CPU work per __getitem__
        self.val_samples = []
        for idx, jp in enumerate(self.json_files):
            rays, rgbs, _ = self.load_data([jp])
            ts = self.all_ids[idx] * torch.ones(rays.shape[0], 1)
            d = sat_utils.read_dict_from_json(jp)
    ...
    def init_scaling_params(self):
        ...
        d = {}
        x_scale, x_offset = sat_utils.rpc_scaling_params(all_points[:, 0])
        d["X_scale"], d["X_offset"] = float(x_scale), float(x_offset)
        y_scale, y_offset = sat_utils.rpc_scaling_params(all_points[:, 1])
    ...
    def load_data(self, json_files, verbose=False):
        ...
        for t, json_p in enumerate(json_files):
            ...
            sun_dirs = self.get_sun_dirs(
                float(d["sun_elevation"]), float(d["sun_azimuth"]), rays.shape[0]
            )
            ...
            if verbose:
                print(
                    "Image {} loaded ( {} / {} )".format(img_id, t + 1, len(json_files))
                )
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            # return preloaded validation sample (avoid repeated IO/compute)
