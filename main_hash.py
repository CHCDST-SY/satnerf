...
from rendering_hash import render_rays
...
from models.loss import sigma_sparsity_loss, total_variation_loss  # 损失函数
...
class NeRF_pl(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        if self.args.model == "sat-nerf":
            ...
            self.use_ts = True
        elif self.args.model == "sat-nerf-hash":
            self.loss_without_beta = SNerfLoss(lambda_sc=args.sc_lambda)
            self.use_ts = True
    ...
    def setup(self, stage=None):
        # 在每个GPU上执行
        # 获取bounding box并定义模型
        if self.models is None:  # 确保模型只定义一次
            # self.bounding_box = self.get_bounding_box_from_dataset()
            self.define_models()
            # 显式迁移子模块到当前设备，避免遗留在 CPU
            current_device = self.device
            if hasattr(self, 'nerf_coarse') and isinstance(self.nerf_coarse, torch.nn.Module):
                self.nerf_coarse.to(current_device)
            if hasattr(self, 'nerf_fine') and isinstance(self.nerf_fine, torch.nn.Module):
                self.nerf_fine.to(current_device)
            if hasattr(self, 'embedding_t') and isinstance(self.embedding_t, torch.nn.Module):
                self.embedding_t.to(current_device)
            # 同步 self.models 字典中的模块（只是二次引用，但确保设备一致）
    ...
    def define_models(self):
        ...
        # hash 版
        if self.args.model == "sat-nerf-hash":
            self.embedding_t = torch.nn.Embedding(
                self.args.t_embbeding_vocab, self.args.t_embbeding_tau
            )
            self.models["t"] = self.embedding_t
            # ------------------ 添加hash版 ------------------
            self.models["coarse_embed_fn"] = self.models["coarse"].hash_embedder
            if self.args.n_importance > 0 :
                self.models["fine_embed_fn"] = self.models["fine"].hash_embedder
    ...
    def transfer_batch_to_device(self, batch, device, dataloader_idx=0):
        # 显式将嵌套 dict/list/tuple 内的所有张量移动到 GPU，避免有张量遗留在 CPU
        def move_to_device(obj):
            ...
            if isinstance(obj, (list, tuple)):
                tp = type(obj)
                return tp(move_to_device(v) for v in obj)
            return obj
    ...
    def configure_optimizers(self):
        ...
        if self.args.model == "sat-nerf-hash":
            # 收集除 hash_embedder 以外的参数，避免与单独的嵌入器参数组重复
            # parameters = []
        ...
        # if self.args.model == "sat-nerf-hash":
        #     if "coarse_embed_fn" in self.models:
        #         param_groups.append(
        #             {"params": self.models["coarse_embed_fn"].parameters(), "eps": 1e-15}
        #         )
        #     if "fine_embed_fn" in self.models:
        #         param_groups.append(
        #             {"params": self.models["fine_embed_fn"].parameters(), "eps": 1e-15}
        #         )
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if self.args.model == "sat-nerf":
            ...
            device = rays.device
            ts = t * torch.ones(rays.shape[0], 1, device=device).long().squeeze()
        elif self.args.model == "sat-nerf-hash":
            t = predefined_val_ts(batch["src_id"][0])
            device = rays.device
            ts = t * torch.ones(rays.shape[0], 1, device=device).long().squeeze()
