...
# Keep BOX_OFFSETS on CPU by default; move to the target device on use
...
def get_voxel_vertices(xyz, bounding_box, resolution, log2_hashmap_size):
    ...
    one_vec = torch.tensor(
        [1.0, 1.0, 1.0], dtype=voxel_min_vertex.dtype, device=voxel_min_vertex.device
    )
    voxel_max_vertex = voxel_min_vertex + one_vec * grid_size

    voxel_indices = bottom_left_idx.unsqueeze(1) + BOX_OFFSETS.to(
        bottom_left_idx.device
    )
