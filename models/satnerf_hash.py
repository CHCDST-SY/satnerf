...
class SatNeRF_hash(torch.nn.Module):
    def __init__(
        self,
        layers=4,
        feat=32,
        ...
        siren=True,
        t_embedding_dims=16,
        sh_degree=4,  # 新增参数：球谐函数的度数
    ):
        super(SatNeRF_hash, self).__init__()
        self.layers = layers
        self.skips = skips
        ...
        # 使用HashEmbedder替换原来的mapping
        if bounding_box is None:
            # 如果没有提供bounding_box，使用默认值
            bounding_box = (
        ...
        # 更新输入特征维度
        in_size = [self.hash_embedder.out_dim, 0]
        ...
        self.fc_net = torch.nn.Sequential(
            *fc_layers
        )  # shared 6-layer structure that takes the encoded xyz vector
        ...
        self.she_encoder = SHEncoder(input_dim=3, degree=sh_degree)

        # 更新输入特征维度
        sun_dir_in_size = [self.she_encoder.out_dim, 0]

        # sun_dir_in_size = 3
        sun_v_layers = []
        sun_v_layers.append(torch.nn.Linear(feat + sun_dir_in_size[0], feat // 2))
        ...
        self.sky_color = torch.nn.Sequential(
            torch.nn.Linear(sun_dir_in_size[0], feat // 2),
            torch.nn.ReLU(),
            torch.nn.Linear(feat // 2, 3),
            torch.nn.Sigmoid(),
        )
    ...
    def forward(
        ...
        # compute shared features using hash encoding
        input_xyz_encoded, keep_mask = self.hash_embedder(input_xyz)
        xyz_ = input_xyz_encoded
        for i in range(self.layers):
            if i in self.skips:
                xyz_ = torch.cat([input_xyz_encoded, xyz_], -1)
        ...
        # zero-out sigma for samples outside the bounding box
        if "keep_mask" in locals() and keep_mask is not None:
            sigma[~keep_mask] = 0.0
        if sigma_only:
            return sigma
        ...
        if self.input_sizes[1] > 0:
            input_xyzdir = torch.cat([xyz_features, input_dir], -1)
        else:
        ...
        sun_dir_encoded = self.she_encoder(input_sun_dir)
        input_sun_v_net = torch.cat([xyz_features, sun_dir_encoded], -1)

        sun_v = self.sun_v_net(input_sun_v_net)
        sky_color = self.sky_color(sun_dir_encoded)
