...
class HashEmbedder(nn.Module):
    def __init__(
        ...
        # 将 bounding_box 统一为形状 [2, 3] 的张量，并注册为 buffer，随设备迁移
        if isinstance(bounding_box, tuple) or isinstance(bounding_box, list):
            try:
                bb = torch.stack(
                    [
                        torch.as_tensor(bounding_box[0], dtype=torch.float32),
                        torch.as_tensor(bounding_box[1], dtype=torch.float32),
                    ],
                    dim=0,
                )
            except Exception:
                bb = torch.as_tensor(bounding_box, dtype=torch.float32)
        else:
            bb = torch.as_tensor(bounding_box, dtype=torch.float32)
        self.register_buffer("bounding_box", bb)
        self.n_levels = n_levels
        self.n_features_per_level = n_features_per_level
        self.log2_hashmap_size = log2_hashmap_size
        # 将分辨率与比例系数注册为 buffer，确保与模型同设备
        self.register_buffer(
            "base_resolution", torch.tensor(base_resolution, dtype=torch.float32)
        )
        self.register_buffer(
            "finest_resolution", torch.tensor(finest_resolution, dtype=torch.float32)
        )
