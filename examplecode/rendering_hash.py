...
def render_rays(models, args, rays, ts):
    ...
    if variant == "s-nerf":
        ...
        result = inference(
            models[typ], args, xyz_coarse, z_vals, rays_d=None, sun_d=sun_d
        )
        if args.sc_lambda > 0:
            # solar correction
    ...
    # hash 版
    elif variant == "sat-nerf-hash":
        from models.satnerf_hash import inference
    ...
    if N_importance > 0:
        ...
        if variant == "s-nerf":
            ...
            # render using main set of rays
            result = inference(
                models[typ], args, xyz_fine, z_vals, rays_d=None, sun_d=sun_d
            )
            if args.sc_lambda > 0:
                # solar correction
                xyz_fine = rays_o.unsqueeze(1) + sun_d.unsqueeze(1) * z_vals.unsqueeze(
                    2
                )  # (N_rays, N_samples, 3)
                result_ = inference(
                    models[typ],
