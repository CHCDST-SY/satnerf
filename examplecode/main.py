...
class NeRF_pl(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        if self.args.model == "sat-nerf-hash":  # 使用哈希编码
            n_levels = self.models["embed_fn"].n_levels  # 哈希表层数
            min_res = self.models["embed_fn"].base_resolution  # 最小分辨率
            max_res = self.models["embed_fn"].finest_resolution  # 最大分辨率
            log2_hashmap_size = self.models["embed_fn"].log2_hashmap_size  # 哈希表大小

            # 对每一层哈希表计算TV损失
            TV_loss = sum(
                total_variation_loss(
                    self.models["embed_fn"].embeddings[i],  # 第i层哈希表权重
                    min_res,
                    max_res,
                    i,
                    log2_hashmap_size,
    ...
    def validation_step(self, batch, batch_nb):
        ...
        self.logger.experiment.add_images(
            "{}_{}/GT_pred_depth".format(split, sample_idx), stack, self.global_step
        )
...
def main():
    ...
    if torch.cuda.is_available():
        trainer_kwargs.update(dict(accelerator="gpu", devices=[args.gpu_id]))
